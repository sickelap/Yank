...
async def totalCaches():
    song_count = (
        len(os.listdir(song_directory)) if os.path.exists(song_directory) else 0
    )
    playlist_count = (
        len(os.listdir(playlist_directory)) if os.path.exists(playlist_directory) else 0
    )
    cache_count = (
        len(os.listdir(cache_directory)) if os.path.exists(cache_directory) else 0
    )
...
async def totalStorage():
    song_size = (
        sum(
            os.path.getsize(f"{song_directory}/{f}") for f in os.listdir(song_directory)
        )
    ...
    playlist_size = (
        sum(
            os.path.getsize(f"{playlist_directory}/{f}")
            for f in os.listdir(playlist_directory)
        )
    ...
    cache_size = (
        sum(
            os.path.getsize(f"{cache_directory}/{f}")
